...
- (Step -1) Install the sublime text editor (optional)
- (Step 0) Install VS Codium (optional)
- (Step 1) Install an Anaconda distribution
- (Step 2) Launch & use Jupyter notebook
- (Step 3) **(only for Windows users)** Install a WSL (Windows Subsystem for Linux) 
...
We don't recommend VS Codium or VS Code, but it is a popular development environment.
...
### Step 2b: Open and run a downloaded jupyter notebook
...
2. Open up the jupyter notebook application with the method of your choice (step 2a)
...
## Step 3 (only for windows users): Install WSL
